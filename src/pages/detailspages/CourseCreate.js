...
export const FinishValidation = ({ lesson, oldStayDuration }) => {
  const [formattedTime, count] = useCountdown(
    (lesson.duration || 0) * 60 - oldStayDuration
  );
      ...
      <div className="row">
        <p>
          {!count ? (
            ...
            <>
              Blog yuzasidan test boshlash uchun blogni o'qib yaxshilab o'qib
              chiqing:{" "}
            </>
          )}
          <button
...
const CourseCreate = () => {
  const { id } = useParams();
  const { getDocument, isAuthenticated, userData, updateUser, updateDocument } =
    useFirebase();
  ...
  useEffect(() => {
    if (
      !userData ||
      !lesson ||
      userData?.lessons?.[id]?.stayDuration === lesson.duration * 60
    ) {
      return;
    }
                  ...
                  <div className="blog-tag-and-share mt--50 d-flex justify-content-end">
                    ...
                    {lesson && (
                      <FinishValidation
                        lesson={lesson}
                        oldStayDuration={
                          userData?.lessons?.[id]?.stayDuration || 0
                        }
                      />
                    )}
                  </div>
